...
#### Merge JSON fields atomically

The default mode replaces the full content. With PostgreSQL resource-scoped working memory, pass `mode: 'merge'` to preserve unrelated fields changed by concurrent merge calls:
...
Merge mode reads and writes inside one storage transaction. Objects merge recursively, arrays replace, omitted fields stay unchanged, and explicit `null` removes a field. Concurrent changes to the same field follow transaction order. Full replacement calls still replace the record, so use merge mode for every writer that needs to preserve concurrent field edits.

The patch and existing content must be JSON objects. Invalid JSON, thread scope, gateway agents, and adapters without atomic working-memory support reject explicit merge calls. The operation doesn't validate values against the configured working-memory schema; validate domain values before calling it. Schema-based working-memory tools automatically use atomic resource updates when supported.
