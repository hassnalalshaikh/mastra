...
import { SkillSearchProcessor, ToolSearchProcessor, createToolSkillPolicy } from '@mastra/core/processors';

const toolPolicy = createToolSkillPolicy({
  generate_image: ['image-generation'],
  edit_image: ['image-generation', 'image-editing'],
  create_artifact: ({ phase, input }) =>
    phase === 'execute' && (input as { type?: string })?.type === 'mini_app' ? ['mini-app-builder'] : [],
});
