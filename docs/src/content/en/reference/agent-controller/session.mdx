...
const receipt = await session.respondToToolSuspensionWithReceipt({
  toolCallId: event.toolCallId,
  resumeData: 'Blue',
  expectedRunId: session.suspensions.get({ toolCallId: event.toolCallId })?.runId,
...
An idle Stop is rejected. Acceptance identifies the current run or an unambiguous parked run when available;
it does not assert that cancellation has finished. Existing local-only abort behavior is preserved.
...
Use matching core, server, and client releases that support command receipts. A newer server paired with an
older core rejects all three commands with `unavailable` before executing them. A newer client receiving a
legacy server response without a receipt reports unconfirmed acceptance and does not resend the command.
