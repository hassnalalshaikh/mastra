...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'The browser automation instance for this session. Set at creation via createSession, or from the AgentController config default. Undefined when no browser is configured or a factory-owned browser no longer matches the current resource and thread.',
  },
]}
/>
...
The forward-only `text_delta` event contains `{ type: 'text_delta', runId, messageId, textDelta }`. It emits assistant text as the native stream processes it, before the run finishes. It excludes reasoning, tool input, and tool results. Each event contains only the new text, not the accumulated message. Subscribe before sending and match `runId` to the accepted command's run. Early events can arrive before the receipt resolves. These events aren't replayed and don't replace the display snapshot or terminal events. Generated text doesn't establish speech playback or successful tool execution.

### Messages and run control

#### `sendMessageWithReceipt({ content, files?, requestContext?, tracingContext?, tracingOptions? })`

Send a user message with the same file conversion as `sendMessage`, and return an immediate delivery handle instead of waiting for the run to finish. Text and JSON attachments are included as fenced text; binary attachments remain file parts. JSON content is document text and isn't parsed as JSON.
...
console.log(delivery.id, accepted.runId, accepted.action)
```

Returns the same `{ id, type, accepted }` handle as `sendSignal`. The `accepted` promise waits for the actual routing decision with `requireDelivery: true` and resolves to `{ accepted: true, runId?, action? }`. Check the action and run ID when your caller requires delivery into a running agent. Acceptance isn't completion. Use session events to observe the run's result.

File conversion can throw before a signal is sent. Routing and stream setup failures reject `accepted`. The method doesn't retry. Existing `sendMessage` behavior is unchanged.
...
Remove one queued follow-up by the `id` a UI read from `displayState.queuedFollowUpItems`. The other queued messages keep their order. Returns whether it was still queued; a follow-up already drained into a run is not affected. Emits `follow_up_queued` with the new `count` and `items`.

```typescript
const [next] = session.displayState.get().queuedFollowUpItems
if (next) session.removeFollowUp({ id: next.id })
```
...
`respondToToolApproval()` and `abort()` return a `SessionCommandReceipt`.
Use `respondToToolSuspensionWithReceipt()` for an answer receipt before the resumed run finishes.
The existing `respondToToolSuspension()` still waits for the resumed run.

```typescript
const receipt = await session.respondToToolSuspensionWithReceipt({
  toolCallId: event.toolCallId,
  resumeData: 'Blue',
  expectedRunId: savedRunId,
  expectedOperationId: session.run.getOperationId(),
})
if (!receipt.accepted) console.log(receipt.reason)
```

The three commands accept optional `expectedRunId` and `expectedOperationId` checks.
Receipts identify the command, accepted target tool/run when available, and whether the native gate accepted it.
Rejection reasons are `no_pending_target`, `stale_target`, `ambiguous_target`, `stopping`, and `unavailable`.
Acceptance is separate from execution, cancellation, artifact saving, and billing completion. Observe native session events for those outcomes.

An unbound idle Stop is rejected. A bound thread Stop accepts cancellation of its captured thread scope,
including native discovery of saved work; it does not assert that saved work exists or that cancellation finished.

The HTTP routes return `{ ok: receipt.accepted, receipt }`. Public client command methods return accepted receipts
and throw `SessionCommandRejectedError` with the rejected receipt. They send commands once: a lost or invalid
response leaves acceptance unconfirmed and is never retried automatically.
...
### `session.followUps.list()`

Return the queued follow-ups in send order, each with a stable `id` and its `content`. The same list is mirrored on `displayState.queuedFollowUpItems` next to the `queuedFollowUps` count, and travels on every `follow_up_queued` event as `items`.

```typescript
const waiting = session.followUps.list()
// [{ id: 'follow-up-1-k3j9x2', content: 'Then propose a fix.' }]
```

### `session.followUps.remove(id)`

Remove one queued follow-up by id without touching the rest of the queue. Returns whether it was still queued. Prefer [`session.removeFollowUp()`](#messages-and-run-control), which also emits the updated queue to subscribers.
...
A `Session` is a live runtime object. Its event bus, arbitrary `session.state`, permission rules, permission grants, ordinary tool suspensions, follow-ups, run state, and stream state don't automatically survive controller or process recreation. The host must restore any of this state when recreating a session.

When a session attaches to a stored thread, it reads saved approvals from the current mode's backing agent, scoped to the same thread and resource. A single saved approval restores `displayState.pendingApproval` and the existing approval gate. Reconnecting doesn't execute the tool or start a model call. Respond through `session.respondToToolApproval()` with the displayed `toolCallId` to approve or decline the original saved run. Respond to that approval or stop its run before sending another message.

Recovery requires native run storage. Multiple saved approvals on the same thread are rejected rather than selecting one arbitrarily. Storage failures remain errors. After a failed decision, the session reads saved work again and can restore its prompt; it never retries the decision automatically. Auto-approval grants remain ephemeral.

For a web client that displays one exact thread, create its client handle with `client.getAgentController(controllerId).session(resourceId, scope, { threadId })`. Use a separate scope for each independent open thread. The client sends `sessionThreadId` on every Session request, so the native server binds the exact stored thread before reading state, opening a stream, or accepting a command. Both client and server must support this query parameter. A missing thread or one belonging to another resource is rejected before a Session is created.

A session scope alone doesn't restore a thread binding after a server restart. Calling `create({ threadId })` before connecting also leaves a gap if the process restarts between requests. The state method's `state({ threadId })` option selects returned thread data; it doesn't provide the same Session binding guarantee.

`create()` uses the handle's exact thread unless an explicit replacement is supplied. Successful `switchThread()`, `createThread()`, and `cloneThread()` commands update that handle's selected thread. Continue refreshing native display and stored messages in `onReconnect`, because the stream doesn't replay missed events.
