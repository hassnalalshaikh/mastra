...
<PropertiesTable
  ...
  },
  {
    ...
    description:
      'Static browser or per-session browser factory. A factory result is owned by its live session and is resolved into agent tools using that session identity.',
    isOptional: true,
  },
  {
    name: 'channels',
...
Create an edited copy of a saved conversation and submit the replacement through a new session. The original thread, later messages, and tool effects remain unchanged. Attachments on the edited message are retained.
...
The source must belong to `resourceId`, and the selected message must be user-authored. Supply an unused thread ID and session scope. The command copies only the earlier stored messages and starts a fresh thread summary. Resource-scoped memory remains shared, as with a new conversation.

The command waits for signal acceptance, not completion of the answer. Read and subscribe to the returned thread using the new scope. If submission fails after the copy was saved, that copy remains available and the original remains intact. Reusing a target is rejected to avoid submitting another run.

The native client exposes the same command as `sourceSession.editMessage({ messageId, content, newThreadId, newSessionScope })`. The source client must have an exact thread binding. Its binding doesn't change when the edited copy is created.
...
### Session browser ownership

A browser factory runs when the controller creates a session. Agent preparation retrieves that same live session's browser; it doesn't invoke the factory again. An explicitly configured agent browser takes precedence. Session identity includes the controller, resource, scope, thread, and live session incarnation, so a deleted session's request context can't resolve a replacement browser.

The controller closes factory-owned browsers when their final owning session is deleted. A failed close keeps the session registered for a later deletion retry. Static controller browsers and explicit session overrides remain caller-owned. Changing a session's resource or thread makes its previous factory browser unavailable to new runs; deletion still closes that retained browser. Keep factories lazy: don't launch a provider session during construction or initialization hooks.
