...
Set `waitingFor: 'external'` when a service callback supplies the resume data:

```typescript
await suspend({ requestId }, { resumeLabel: 'callback', waitingFor: 'external' })
```

The native suspended step result stores `waitingFor` alongside `suspendPayload`.
Omitting the option means `'user'`; older snapshots without this field also mean
`'user'`. This label does not schedule work, resume a step, or change approval
checks. Use the same native resume API when the callback arrives.
