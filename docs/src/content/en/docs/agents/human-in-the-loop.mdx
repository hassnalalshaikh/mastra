...
Tools waiting for a service callback can distinguish that wait from a question
for the user:
...
The option defaults to `'user'`. It is preserved in native suspension events,
saved tool metadata, `listSuspendedRuns`, and Session `pendingSuspensions`.
It does not change approval checks or allow a second independent run on the
same thread. A same-agent message accepted while suspended is queued into the
existing run and is processed after that run resumes; its delivery receipt is
not a separate task-completion receipt.
