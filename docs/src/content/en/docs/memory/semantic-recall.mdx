...
### Separate query and passage embedders

Asymmetric retrieval models can require different transformations for search queries and stored content. Set `embedder` to the passage model and `queryEmbedder` to the matching query model. If `queryEmbedder` is omitted, semantic recall uses `embedder` for both roles.

```ts
const memory = new Memory({
  storage,
  vector,
  embedder: passageEmbeddingModel,
  queryEmbedder: queryEmbeddingModel,
})
```

Both options accept any AI SDK-compatible embedding model and must produce embeddings with the same dimensions. Use `embedderOptions` for stored content and `queryEmbedderOptions` for queries when the provider requires role-specific options.
