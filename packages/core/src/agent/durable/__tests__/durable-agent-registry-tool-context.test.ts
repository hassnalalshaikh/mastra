...
describe('durable registry tool execution context', () => {
  it.each([
    ...
    { advertised: true, key: 'publishedWorkflowTool' },
    { advertised: false, key: 'runWorkflow' },
    { advertised: false, key: 'publishedWorkflowTool' },
  ])(
    'requires an advertised schema and retains workflow context across turns ($advertised, $key)',
    async ({ advertised, key }) => {
      let modelCalls = 0;
      ...
      try {
        for (const turn of [1, 3]) {
          ...
          if (!advertised) {
            expect(chunks.filter(chunk => chunk.type === 'tool-error')).toHaveLength(1);
            expect(chunks.find(chunk => chunk.type === 'tool-error')?.payload.error.name).toBe('ToolNotFoundError');
            continue;
          }
        ...
        expect(workflowInputs).toEqual(advertised ? ['request-1', 'request-3'] : []);
        expect(identities).toEqual(
          (advertised ? [1, 3] : []).map(turn => ({
