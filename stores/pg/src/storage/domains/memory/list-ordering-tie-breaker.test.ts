...
describe('MemoryPG list ordering with equal timestamps', () => {
  ...
  it('orders thread messages by creation time and then by id in the same direction', async () => {
    ...
    expect(lastQuery(client)).toContain('ORDER BY COALESCE("createdAtZ", "createdAt") ASC, "id" ASC');
    ...
    expect(lastQuery(client)).toContain('ORDER BY COALESCE("createdAtZ", "createdAt") DESC, "id" DESC');
  });
